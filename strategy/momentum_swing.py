...
on_candle only accumulates daily-close history per symbol and never emits a
signal itself -- the actual entries/exits come out of compute_rebalance,
called once a month by the caller (see main.py's run_backtest_swing /
run_swing_live_or_paper), since this strategy's logic is inherently
cross-sectional rather than a single symbol's per-candle reaction.
"""
...
class MomentumSwingStrategy(Strategy):
    ...
    def on_candle(self, symbol: str, candle: dict) -> Signal | None:
        self._state_for(symbol).closes.append(candle["close"])
        return None
